...
namespace collect_agent {
   ...
   */
  DLL_PUBLIC std::string store_files(
      long long timestamp,
      const std::string& suffix,
      int n_filepaths,
  ...
  /*
   * Reset rstats internal state essentially boiling
   * down to restarting it.
   */
  DLL_PUBLIC std::string restart_rstats();
}
 ...
 * json.hpp
 *
 * Adapted from https://github.com/nbsdx/SimpleJSON
 */
        ...
        string serialize() const {
            switch( Type ) {
                ...
                case Class::Object: {
                    ...
                    string s = "{";
                    for( auto &p : *Internal.Map ) {
                        s += ( sep + p.first + "\":" + p.second.serialize() );
                        sep = ",\"";
                    }
                    s += "}";
                    return s;
